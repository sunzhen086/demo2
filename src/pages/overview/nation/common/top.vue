<template>
	<div class="main-top">
		...
		<div class="top-mid left">
		    ...
		    <!--<div class="realTimeBtn" :class="{blinkStatus:realTimeBtnStatus}" @click="showTable">
		    	<i class="fa fa-clock-o" aria-hidden="true"></i> 今日实时
		    </div>-->
		</div>
	</div>
</template>
...
<script>
	export default {
		...
		methods:{
			...
			realTimeBtnBlink: function() {
				var that = this;
				var timer = this.timer = setInterval(function() {
					that.realTimeBtnStatus = !that.realTimeBtnStatus;
				},2000);
			}
		},
		mounted() {
			this.realTimeBtnBlink();
		},
		beforeDestroy() {
...
<style lang="scss" scoped>
	...
	.realTimeBtn {
		padding: 0rem 0.1rem;
		background-color: rgba(6,192,157,1);
		float: left;
		cursor: pointer;
		font-weight: bold;
		margin-left: 0.3rem;
	...
	.blinkStatus {
		background-color: rgba(6,192,157,0.4);
	}
</style>
